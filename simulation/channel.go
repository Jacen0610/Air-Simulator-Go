...
// ForceTransmit 强制发送高优先级消息，绕过CSMA竞争
...
// AttemptTransmit 改为同步阻塞模型，函数在传输完成或被破坏后才返回。
// 返回值: (是否成功, 碰撞源ID)
// 如果成功，碰撞源ID为空字符串。
// 如果失败，碰撞源ID为当前占用信道的发送者ID，或者是 "COLLISION" (如果传输中途被破坏)。
func (c *Channel) AttemptTransmit(msg ACARSMessageInterface, senderID string, transmissionTime time.Duration) (bool, string) {
	// 为本次传输尝试生成一个唯一的ID。
...
// jamChannel 用于在碰撞后将信道标记为拥塞，并在一段时间后清除。
